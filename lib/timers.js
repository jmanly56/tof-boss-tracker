/**
 * @module timers
 * Creates and manages the boss timers.
 */
        ...
        if (Object.keys(timers.data).length === 0) {
            Object.keys(BossInfo).forEach((x) => {
                timers.setValue(x, {
                    ...
                    Users: new Array(BossInfo[x].numChannels),
                });
            });
            timers.save();
        }
...
function createMessage(boss) {
    let x = timers.data[boss]['Timers'].map((cur, i) => {
        return `Channel: ${i + 1}${i < 9 ? '  ' : ' '}Last Kill: <t:
        ${cur != null ? Math.floor(new Date(cur)) / 1000 : 'None'}
        :t> Updated by: <@!${timers.data[boss]['Users'][i]}>\n`;
    });
    return x.join('');
}
